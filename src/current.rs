...
vvar_data! {
    ...
    /// 每个CPU初始化时由`TrapInfo::new_handler`创建的专用任务，仅在调度器
    /// 没有可运行任务时作为上下文容器，用于储存中断上下文，不进入普通就绪队列。
    ///
    /// 使用`init_task`作为等待上下文会使其栈和上下文被WFI唤醒流程覆盖，
    /// 因此这里使用专用任务；`new_handler`的参数传入空指针，若该任务意外
    /// 被调度运行，会因`trap_handler`收到空指针而快速定位问题。
