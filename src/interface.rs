...
trait_interface! {
    pub trait UserData {
        ...
        ///
        /// 该接口用于在内核态初始化用户调度器中的自引用和函数指针。`pos`可以是
        /// 内核vDSO中的地址A，也可以是内核访问用户共享对象时使用的地址C；返回
        /// 同一物理对象在目标用户地址空间中的地址B。
        ///
        /// 也就是说，该接口支持`A -> B`和`C -> B`两种转换。
        ///
        /// 事件源注册只在初始化或注册路径调用该接口，调度热路径使用已经保存的
        /// 用户态和内核态地址，不会重复执行地址转换。
        ///
        /// 若无法确认整个`[pos, pos + len)`在目标地址空间中存在唯一且连续的对应
        /// 映射，则返回空指针。
